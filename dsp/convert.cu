...
CudaLaunchStatus launchStatusFrom(cudaError_t error) noexcept {
  return error == cudaSuccess ? CudaLaunchStatus::success : CudaLaunchStatus::launchFailure;
}

bool invalidLaunchArgs(const void* input, const void* output, std::size_t elementCount) noexcept {
  return elementCount > 0 && (input == nullptr || output == nullptr);
}

}  // namespace

CudaLaunchStatus k0IngressConvertCuda(const common::ci16* input, common::cf32* output,
                                       std::size_t elementCount, void* stream) noexcept {
  if (elementCount == 0) {
    return CudaLaunchStatus::success;
  }
  if (invalidLaunchArgs(input, output, elementCount)) {
    return CudaLaunchStatus::invalidArgument;
  }

  k0IngressConvertKernel<<<gridSize(elementCount), kTile, 0, toCudaStream(stream)>>>(
      input, output, elementCount);
  return launchStatusFrom(cudaGetLastError());
}
